...
(function(){
  ...
  const CONFIG={
    ...
    backupCookieUrl:"https://sayan10-sportlink-cookies.pages.dev/api/cookie.json"
  };
  document.addEventListener("DOMContentLoaded",async()=>{
    ...
    let cookieValue="";
    try{ const response=await fetch(CONFIG.cookieUrl,{cache:"no-store"}); if(!response.ok) throw new Error(); const data=await response.json(); cookieValue=data.cookie||""; }catch(e){ try{ const response2=await fetch(CONFIG.backupCookieUrl,{cache:"no-store"}); const data2=await response2.json(); cookieValue=data2.cookie||""; }catch(e2){} }
    if(cookieValue){
